...
const post = async (req, res) => {
    ...
    if( await getSingle({ name: data?.name }) ){
        return GeneralConrtller.ResponseObj( res, 400, "Subject Name Repeated", null, false)   
    }
    if( await getSingle({ subCode: data?.subCode }) ){
        return GeneralConrtller.ResponseObj( res, 400, "Subject Code Repeated", null, false)   
    }
    ...
    try {
        ...
        return GeneralConrtller.ResponseObj( res, 200, "Subject added Successfully", createdSubject, true) 
    } catch (error) {
...
const update = async (req, res) => {
    ...
    if( await getSingle({ name: data?.name, _id : { $ne : data?._id } }) ){
        return GeneralConrtller.ResponseObj( res, 400, "Subject Already Exist", null, false)   
    }
    ...
    try {
        ...
        return GeneralConrtller.ResponseObj( res, 200, "Subject Updated Successfully", updated, true) 
    } catch (error) {
        return GeneralConrtller.ResponseObj( res, 400, "Failed", error, false)   
    }
}
...
const get = async (req, res) => {
    ...
    return GeneralConrtller.ResponseObj( res, data?.length > 0 ? 200: 400, "Subjects", data, data?.length > 0 ? true: false)   
}

const deleteList = async (req, res) => {
    let data = await Subject_Model.deleteMany();
    return GeneralConrtller.ResponseObj( res, 200, "Subjects", [], true)   
}
